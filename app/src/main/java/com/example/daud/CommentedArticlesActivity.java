...
public class CommentedArticlesActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_commented_articles);

        boolean isNightMode = getIntent().getBooleanExtra("nightMode", false);
        applyNightMode(isNightMode);

        findViewById(R.id.btnBack).setOnClickListener(v -> finish());
        RecyclerView rv = findViewById(R.id.rvCommentedArticles);
        ...
        if (userId != -1) {
            viewModel.getCommentedArticles(userId).observe(this, articles -> {
    ...
    private void applyNightMode(boolean isNightMode) {
        ...
        findViewById(R.id.commentedRoot).setBackgroundColor(bgColor);
